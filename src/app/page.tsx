    ...
    <main className="min-h-screen bg-slate-100 text-slate-950">
      ...
      <div className="mx-auto grid max-w-7xl gap-4 px-4 py-4 lg:grid-cols-[260px_1fr]">
        ...
        <section className="space-y-4 pb-28 lg:pb-4">
          ...
          {view === "Settings" && <SettingsView retention={retention} setRetention={setRetention} role={role} openAiApiKey={openAiApiKey} setOpenAiApiKey={setOpenAiApiKey} resetDemo={() => { localStorage.removeItem(storageKey); setDrums(demoDrums); setProducts(demoProducts); setOpenAiApiKey(""); setSelectedDrumId(demoDrums[0].id); }} />}
        </section>
      </div>
...
function SettingsView({ retention, setRetention, role, openAiApiKey, setOpenAiApiKey, resetDemo }: { retention: RetentionPolicy; setRetention: (value: RetentionPolicy) => void; role: Role; openAiApiKey: string; setOpenAiApiKey: (value: string) => void; resetDemo: () => void }) {
  return <Panel title="Settings"><div className="grid gap-4"><Label text="Image retention"><select className="input" value={retention} onChange={(e) => setRetention(e.target.value as RetentionPolicy)}><option>Delete after processing</option><option>Keep until drum completion</option><option>Keep permanently</option></select></Label><Label text="OpenAI Vision API key"><input className="input" type="password" placeholder="sk-..." value={openAiApiKey} onChange={(e) => setOpenAiApiKey(e.target.value)} autoComplete="off" /></Label><p className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">OCR engine: {openAiApiKey ? "OpenAI Vision first, Tesseract fallback" : "Tesseract fallback only"}. API keys are stored only in this browser for this static site.</p><p className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">Current policy: {retention}. Drum, image preview, product, OCR, and setting changes are saved in this browser automatically.</p><button onClick={resetDemo} className="touch-button justify-center border border-slate-300 bg-white"><RotateCcw size={20} />Reset demo data</button><div className="rounded-lg border border-slate-200 p-3"><strong>Role permissions</strong><p className="text-sm text-slate-600">{role}: {role === "Chemist" ? "Create, scan, edit active drums, complete drums, view history." : role === "Supervisor" ? "Chemist permissions plus review all drums." : "Full access including users, products, and settings."}</p></div><Panel title="Audit trail sample"><div className="space-y-2 text-sm">{demoAudit.map((event) => <p key={event.id}><strong>{event.action}</strong> · {event.drumId} · {event.user} · {new Date(event.timestamp).toLocaleString()}</p>)}</div></Panel></div></Panel>;
}