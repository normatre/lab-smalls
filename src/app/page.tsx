...
import type { Drum, InventoryItem, OcrCategories, PhysicalState, Product, RetentionPolicy, Role, Unit } from "@/types/lab-smalls";
...
function OcrOutput({ text, categories }: { text?: string; categories?: OcrCategories }) {
  const groups = categories ? [
    ["Chemical candidates", categories.chemicalCandidates], ["Brand", categories.brands],
    ["Catalogue", categories.catalogNumbers], ["CAS", categories.casNumbers],
    ["Pack size", categories.packageSizes], ["Lot", categories.lotNumbers],
...
function ProductDatabase({ products }: { products: Product[] }) {
  return <Panel title="Chemical Databases"><div className="grid gap-3 md:grid-cols-3"><div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4"><Database className="mb-3 text-emerald-800" size={28} /><h3 className="font-bold text-emerald-950">Verified identity lookup</h3><p className="mt-2 text-sm text-emerald-900">OCR results are checked in order: Sigma-Aldrich first, Merck second, then PubChem. Catalogue codes are used whenever they are visible.</p></div><div className="rounded-lg border border-slate-200 bg-white p-4"><ClipboardCheck className="mb-3 text-slate-700" size={28} /><h3 className="font-bold">Only required inventory fields</h3><p className="mt-2 text-sm text-slate-600">Database results improve the chemical name and physical state. The inventory keeps only name, quantity, container size, unit and physical state.</p></div><div className="rounded-lg border border-amber-200 bg-amber-50 p-4"><AlertTriangle className="mb-3 text-amber-800" size={28} /><h3 className="font-bold text-amber-950">Closest OCR fallback</h3><p className="mt-2 text-sm text-amber-900">If no trusted database confirms the candidate, the strongest OCR name and the label size are still added as Review Required.</p></div></div><section className="mt-5"><h3 className="mb-3 text-base font-bold">Local fallback products</h3><div className="grid gap-3">{products.length === 0 && <p className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm font-semibold text-slate-600">No local fallback products are saved in this browser.</p>}{products.map((product) => <div key={product.id} className="rounded-lg border border-slate-200 bg-white p-4"><strong>{product.canonicalName}</strong><p className="mt-1 text-sm text-slate-600">{product.typicalContainerSize ?? "-"} {product.unit ?? ""} · {product.physicalState}</p></div>)}</div></section></Panel>;
}
